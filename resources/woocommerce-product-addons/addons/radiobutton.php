...
<?php /* if($addon['name'] == 'Substrate' && get_field('substrate_options', 'options')) {
  $sOptions = get_field('substrate_options', 'options');


  foreach ( $sOptions as $i => $option ) {
		$loop++;

		$price        = !empty( $option['price_per_sqft'] ) ? $option['price_per_sqft'] : '';
		...
		$price_display = WC_Product_Addons_Helper::get_product_addon_price_for_display( $price );
      ...
      <?php echo !empty($option['option_description']) ? "<p class='option-description'>{$option['option_description']}</p>" : ''; ?>
		</p>
  <?php }
