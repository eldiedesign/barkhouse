...
<div class="wc-pao-addon-container <?php echo $required ? 'wc-pao-required-addon' : ''; ?> wc-pao-addon wc-pao-addon-<?php echo sanitize_title( $name ); ?>" data-product-name="<?php echo esc_attr( $product_title ); ?>">
	...
	?>
	<?php
	if ( $display_description || ($addon['name'] == 'Substrate' && get_field('substrate_description', 'options') ) ) {
		?>
		<?php echo $addon['name'] == 'Substrate' && get_field('substrate_description', 'options') ? '<div class="wc-pao-addon-description">' . get_field('substrate_description', 'options') . '</div>' : '<div class="wc-pao-addon-description">' . wpautop( wptexturize( $description ) ) . '</div>'; ?>
	<?php }; ?>
